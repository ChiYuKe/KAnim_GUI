...
GIF 动画导出内置 FFmpeg 8.1.2，并使用 `palettegen` / `paletteuse` 和 Lanczos 缩放改善颜色、抖动和缩放质量。FFmpeg 压缩包会随发布包复制到 `Resources/ffmpeg`，首次导出时解压到当前用户的 KAnimGui 工具缓存；导出的临时 PNG 和调色板会在完成后自动删除。FFmpeg 的许可证见 `KAnimGui/Resources/ffmpeg/LICENSE-FFMPEG.txt`。
