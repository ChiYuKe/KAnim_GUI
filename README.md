...
GIF 动画导出内置 FFmpeg 8.1.2，并使用 `palettegen` / `paletteuse` 和可选缩放算法（Lanczos、Bicubic、Spline、Nearest）改善颜色、抖动和边缘质量。播放速度、输出尺寸、缩放算法、输出目录和完成通知选项会持久化到 `%LocalAppData%/KAnimGui/settings.json`。应用设置不再依赖 `KAnimGui.dll.config`；首次启动会自动迁移旧 `user.config` 中的已知选项，因此旧配置文件损坏也不会阻止程序启动。FFmpeg 压缩包会随发布包复制到 `Resources/ffmpeg`，首次导出时解压到当前用户的 KAnimGui 工具缓存；导出的临时 PNG 和调色板会在完成后自动删除。FFmpeg 的许可证见 `KAnimGui/Resources/ffmpeg/LICENSE-FFMPEG.txt`。
